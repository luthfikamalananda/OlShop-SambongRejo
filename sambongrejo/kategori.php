...
<head>
    ...
    <title>Sambong Rejo Kategori</title>
...
<body>
    ...
    <!-- Banner Ends Here -->
    <?php //  <!-- untuk mengambil digit di website terakhir -->
    $query = $_SERVER['QUERY_STRING'];
    $idbarang = substr($query, 3);
    if (isset($_GET['id'])) {
        $id_berita = $_GET['id'];
    } else {
    ...
    <?php
    $sql = mysqli_query($con, "SELECT * FROM kategori_produk where id=$id_berita ");
    while ($hasil = mysqli_fetch_array($sql)) {
        $id = stripslashes($hasil['id']);
        $nama  = stripslashes($hasil['nama']);
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="section-heading">
                            <h2>Kategori <?php echo $hasil['nama'] ?></h2>
                        <?php } ?>
                        <a href="promo.php">lihat promo <i class="fa fa-angle-right"></i></a>
                        </div>
                    </div>
                    <?php
                    $sql = mysqli_query($con, "SELECT * FROM produk where id_kategori = $idbarang order by id DESC");
                    $nomer = 1;
                    while ($hasil = mysqli_fetch_array($sql)) {
                        $id = $hasil['id'];
                        $nama  = stripslashes($hasil['nama']);
